/*JANGAN LUPA LIKE+COMEN AND SKIRIKEP
*YOUTUBE ARDIGANS YT ❤️🔥
*/
const {
    WAConnection,
    MessageType,
...
prefix = "!"
name = "PATRICKSTAR"
rdaftar = "TERIMA KASIH TELAH MENDAFTARNAMA ANDA TELAH TERDAFTAR DI DATABASE PATRICK😁"
rmenu = "HAI BRO JANGAN LUPA DONASI YAA🐦"
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["6285282996146@s.whatsapp.net"] // ubah aja gapapa
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 Patrick BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 Patrick BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
