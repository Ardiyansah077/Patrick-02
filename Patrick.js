...
const { help, bahasa, donasi, limitend, limitcount, bottt } = require('./Patrick/help')
...
prefix = "!"
name = "PATRICK"
rdaftar = "TERIMA KASIH TELAH DAFTAR Nama Anda Sudah Terdaftar Di Database Patrick😆"
rmenu = "HAI ${name} JANGAN LUPA DONASI YAA😁"
botinfo = "Jangan Menambahkan Patrick Kegrup Tanpa Izin Bro😊"
limitt = 50
memberLimit = 50
ban = []
premium = ["6285282996146@s.whatsapp.net","62852829961460@s.whatsapp.net"]
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','red'), color('!','yellow'), color(']','red'), color(' Scan Tu Bro Jangan Lupa Subscribe ArdiGansYT', 'red'))
	})
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Patrick.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ArdiGans = ["6285282996146@s.whatsapp.net"] // ubah aja gapapa
			...
			const ownerNumber = ["6285282996146@s.whatsapp.net","62852829961460@s.whatsapp.net"] // owner number ubah aja
...
//akhir kreator
		    ...
		    case 'darkjokes':
				...
				reply(mess.wait)
				 data = fs.readFileSync('./Patrick/drak.js');
		    ...
		    case 'map':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 PatrickStar 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 PatrickStar 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
